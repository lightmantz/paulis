...
class StockTakeController extends Controller
{
    public function index(Request $request)
    {
        ...
        $stats = compact(
            'systemUnits', 'systemValue', 'physicalUnits', 'physicalValue',
            'shortageUnits', 'surplusUnits', 'lostValue'
        );
    ...
    public function destroy(StockTake $take)
    {
        if ($take->status === 'posted') {
            return back()->with('error', 'A posted stock take cannot be deleted.');
        }
        if (auth()->user()->role !== 'owner') {
            return back()->with('error', 'Only the Business Owner can delete a stock take.');
        }
        ...
        activity('stock-take')->log("Deleted stock take draft {$reference}");

        return redirect()->route('business.stock-taking')
            ->with('success', "Stock take {$reference} deleted.");
    }
